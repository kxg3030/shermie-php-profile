d3 = function () {
    ...
    d3.format           = function (specifier) {
        var match                                                                                    = d3_format_re.exec(specifier), fill = match[1] || " ", align                      = match[2] || ">",
            ...
            comma = match[7], precision = match[8], type = match[9], scale = 1, suffix = "", integer = false;
    ...
    d3.svg.brush                                     = function () {
        ...
        function brushstart() {
            var target = this, eventTarget = d3.select(d3.event.target), event_ = event.of(target, arguments),
                ...
                dragging                                                        = eventTarget.classed("extent"), center, origin        = mouse(), offset;
    ...
    var d3_layout_stackOffsets = d3.map({
                                            silhouette: function (data) {
                                                var n = data.length, m = data[0].length, sums = [], max = 0, i, j, o,
                                                    y0                                                  = [];
                                                for (j = 0; j < m; ++j) {
                                            ...
                                            wiggle    : function (data) {
                                                var n = data.length, x = data[0], m = x.length, i, j, k, s1, s2,
                                                    s3, dx,
                                                    o, o0, y0                       = [];
                                                y0[0]                               = o = o0 = 0;
                                                for (j = 1; j < m; ++j) {
        ...
        function resampleLineTo(x0, y0, λ0, a0, b0, c0, x1, y1, λ1, a1, b1, c1, depth, stream) {
            ...
            if (d2 > 4 * δ2 && depth--) {
                var a                                                                           = a0 + a1, b = b0 + b1, c = c0 + c1, m                                    = Math.sqrt(a * a + b * b + c * c),
                    φ2                                                                          = Math.asin(c /= m), λ2                                                  = Math.abs(Math.abs(c) - 1) < ε ? (λ0 + λ1) / 2 : Math.atan2(b, a),
    ...
    function d3_geo_projectionMutator(projectAt) {
        var project, rotate, projectRotate, projectResample         = d3_geo_resample(function (x, y) {
            ...
            δγ                                                      = 0, δx, δy, clip = d3_geo_clipAntimeridian,
            clipAngle                                               = null;
